...
import {
  ...
  getAdminClubs, deactivateClub, activateClub, deleteClubPermanent,
...
export default function AdminPage() {
  ...
  const handleActivate = async (clubId, clubName) => {
    if (!window.confirm(`"${clubName}" 동아리를 활성화하시겠습니까?`)) return;
    try { await activateClub(clubId); load(getAdminClubs, setClubs); }
                  ...
                  <td style={s.td}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      {c.isActive === 'Y' ? (
                        ...
                        <button style={s.activateBtn} onClick={() => handleActivate(c.clubId, c.clubName)}>
                          활성화
                        </button>
                      )}
...
const s = {
  ...
  activateBtn: { padding: '6px 12px', background: '#d1fae5', color: '#065f46', border: 'none', borderRadius: 6, fontSize: 13, cursor: 'pointer', fontWeight: 600 },
